...
from utils.k_fold_cv import k_fold_cross_validation 
...
C_values = np.logspace(-5, 5, num=10)  # 10 values for each y

# Dictionaries to store best C and errors for each y
best_C_per_gamma = {}
cv_scores_per_gamma = {}
training_errors = []
test_errors = []
...
for gamma in gamma_values:
    ...
    for C in C_values:
        ...
        print(f"y={gamma:.6f}, C={C:.6f}, CV Accuracy={mean_score:.4f}")

    # Select best C for this y
    best_C = max(C_scores, key=C_scores.get)
    best_C_per_gamma[gamma] = best_C
    cv_scores_per_gamma[gamma] = C_scores[best_C]

    # Train final model on full training set with best C_y
    ...
    print(f"y={gamma:.6f}, Best C={best_C:.6f}, Training Error={train_error:.4f}, Test Error={test_error:.4f}")
...
print("Best y and C saved for later training.")
